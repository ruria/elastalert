...
class ElastAlerter():
    ...
    def writeback(self, doc_type, body):
        ...
        if self.writeback_es:
            try:
                res = self.writeback_es.create(index=self.writeback_format(self.writeback_index, self.writeback_index_fmt, self.writeback_es),
    ...
    def get_aggregated_matches(self, _id):
        ...
        if self.writeback_es:
            try:
                res = self.writeback_es.search(index=self.writeback_format(self.writeback_index, self.writeback_index_fmt,self.writeback_es),
                                               doc_type='elastalert',
                                               body=query,
                                               size=self.max_aggregation)
                for match in res['hits']['hits']:
                    matches.append(match['_source'])
                    self.writeback_es.delete(index=self.writeback_format(self.writeback_index, self.writeback_index_fmt,self.writeback_es),
                                             doc_type='elastalert',
                                             id=match['_id'])
