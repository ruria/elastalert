...
class ElastAlerter():
    ...
    def writeback_format(writeback_index,writeback_index_fmt):
    ...
    def writeback(self, doc_type, body):
        ...
        if self.writeback_es:
            try:
                res = self.writeback_es.create(index=writeback_format(writeback_index,writeback_index_fmt),
    ...
    def get_aggregated_matches(self, _id):
        ...
        if self.writeback_es:
            try:
                res = self.writeback_es.search(index=writeback_format(writeback_index,writeback_index_fmt),
                                               doc_type='elastalert',
                                               body=query,
                                               size=self.max_aggregation)
                for match in res['hits']['hits']:
                    matches.append(match['_source'])
                    self.writeback_es.delete(index=writeback_format(writeback_index,writeback_index_fmt),
                                             doc_type='elastalert',
                                             id=match['_id'])
