...
class ElastAlerter():
    ...
    def writeback_format(self, writeback_index, writeback_index_fmt):
    ...
    def get_starttime(self, rule):
        ...
        try:
            if self.writeback_es:
                res = self.writeback_es.search(index=self.readback_index, doc_type='elastalert_status',
    ...
    def writeback(self, doc_type, body):
        ...
        if self.writeback_es:
            try:
                res = self.writeback_es.create(index=self.writeback_format(self.writeback_index, self.writeback_index_fmt),
    ...
    def find_recent_pending_alerts(self, time_limit):
        ...
        if self.writeback_es:
            try:
                res = self.writeback_es.search(index=self.readback_index,
                                               doc_type='elastalert',
                                               body=query,
                                               size=1000)
                if res['hits']['hits']:
    ...
    def send_pending_alerts(self):
        ...
        for alert in pending_alerts:
            ...
            if ts_now() > ts_to_dt(alert_time):
                ...
                try:
                    self.writeback_es.delete(index=self.readback_index,
                                             doc_type='elastalert',
                                             id=_id)
                except:  # TODO: Give this a more relevant exception, try:except: is evil.
    ...
    def get_aggregated_matches(self, _id):
        ...
        if self.writeback_es:
            try:
                res = self.writeback_es.search(index=self.writeback_format(self.writeback_index, self.writeback_index_fmt),
                                               doc_type='elastalert',
                                               body=query,
                                               size=self.max_aggregation)
                for match in res['hits']['hits']:
                    matches.append(match['_source'])
                    self.writeback_es.delete(index=self.writeback_format(self.writeback_index, self.writeback_index_fmt),
                                             doc_type='elastalert',
                                             id=match['_id'])
    ...
    def is_silenced(self, rule_name):
        ...
        if self.writeback_es:
            try:
                res = self.writeback_es.search(index=self.readback_index, doc_type='silence',
